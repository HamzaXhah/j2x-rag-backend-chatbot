...
| **LLM/Embeddings** | Alibaba Cloud Qwen 3.7 Plus API / local BGE Large |
...
cp .env.example .env
# Edit .env with your Alibaba Cloud Model Studio API key
...
- About 1.3 GB of disk space for `BAAI/bge-large-en-v1.5`
...
# Download BGE Large to backend/models
...
### Alibaba Cloud Qwen and local BGE embeddings
```env
DASHSCOPE_API_KEY=your-alibaba-cloud-model-studio-api-key
DASHSCOPE_BASE_URL=https://dashscope-intl.aliyuncs.com/compatible-mode/v1
QWEN_MODEL=qwen3.7-plus
BGE_MODEL_NAME=BAAI/bge-large-en-v1.5
BGE_MODEL_PATH=./models/bge-large-en-v1.5
BGE_DEVICE=cpu
...
2. **Alibaba Cloud Model Studio Errors**
   - Check `DASHSCOPE_API_KEY` and ensure the base URL matches the key's region
   - Verify that `qwen3.7-plus` is enabled for the account and region
   - For SSL issues in dev: set `DISABLE_SSL_VERIFICATION=true`

3. **Embedding dimension errors**
   - BGE Large produces 1024-dimensional vectors; existing OpenAI/Azure vectors cannot be reused
   - Reset the vector database and upload/index documents again after migrating
