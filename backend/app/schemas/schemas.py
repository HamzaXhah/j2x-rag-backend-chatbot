...
class QueryRequest(BaseModel):
    ...
    conversation_history: Optional[List[Dict[str, str]]] = Field(
        None,
        description=(
            "Complete current user and assistant thread used to resolve "
            "follow-ups and preserve analysis context"
        ),
    )
    ...
    def validate_conversation_history(
        ...
        for turn in v:
            ...
            validated_turn = {"role": role, "content": content}
            resolved_question = str(
                turn.get("resolved_question", "")
            ).strip()
            if resolved_question:
                validated_turn["resolved_question"] = resolved_question
...
class QuerySuggestionRequest(BaseModel):
    document_ids: Optional[List[str]] = Field(
        None,
        max_length=20,
        description="Indexed documents used to create investor-focused questions",
    )
    limit: int = Field(25, ge=3, le=30)

    @field_validator('document_ids')
    @classmethod
    def validate_suggestion_document_ids(
