...
class DocumentParser:
    ...
    def parse_pptx(self, file: BinaryIO, metadata: Dict[str, Any] = None) -> List[Document]:
        """Extract visible text from each non-empty PowerPoint slide."""
        try:
            buffer = BytesIO(file.read())
            documents = []

            with zipfile.ZipFile(buffer) as presentation:
                slide_names = [
                ...
                for slide_num, slide_name in enumerate(slide_names, start=1):
                    ...
                    documents.append(
                        Document(page_content=text, metadata=slide_metadata)
                    )

            logger.info(
                "Parsed %s non-empty slides from PPTX", len(documents)
            )
            return documents
    ...
    def parse(self, file: BinaryIO, metadata: Dict[str, Any] = None) -> List[Document]:
        ...
        if file_format == "application/pdf" or file_format.endswith("/pdf"):
            ...
            return self.parse_pptx(file, metadata)
        else:
